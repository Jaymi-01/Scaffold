...
// Link removed to resolve unused imports
...
function VerifyOtpContent() {
  ...
  useEffect(() => {
    if (lockoutTimer <= 0) {
      if (lockoutActive) {
        Promise.resolve().then(() => setLockoutActive(false));
      }
      return;
    }
    const interval = setInterval(() => {
      ...
      <form onSubmit={handleVerify} className="space-y-4">
        <div className="flex justify-between gap-2" onPaste={handlePaste}>
          {otp.map((digit, index) => (
            <input
              ...
              className={`w-11 h-11 text-center text-lg font-bold rounded-xl bg-golden-chestnut-50 text-golden-chestnut-950 outline-none transition border focus:ring-2 focus:ring-rosy-copper-600/10 ${
                errors.form
                  ? "border-oxblood-500 focus:border-oxblood-600 focus:ring-oxblood-600/20"
                  : "border-golden-chestnut-200 focus:border-rosy-copper-600"
              }`}
            />
          ))}
        </div>
        ...
        <div className="text-right min-h-[20px]">
          {lockoutActive && lockoutTimer > 0 ? (
            <p className="text-[10px] text-oxblood-600 font-semibold animate-shake">
              Locked out for {Math.floor(lockoutTimer / 60)}:{(lockoutTimer % 60) < 10 ? "0" : ""}{lockoutTimer % 60}
            </p>
          ) : errors.form ? (
            <p className="text-[10px] text-oxblood-600 font-semibold animate-shake">
              {errors.form}
            ...
            <p className="text-[10px] text-oxblood-600 font-semibold">
        ...
        <div className="flex justify-between items-center text-xs pt-3 border-t border-golden-chestnut-100">
          ...
          <button
            ...
            className={`font-semibold transition ${
              lockoutActive
                ? "text-graphite-350 cursor-not-allowed"
